...
const RUBY_IMAGE_TAG = process.env.RUBY_IMAGE_TAG!;

export class RubyExecutor implements Executor {
    async execute(options: ExecutionOptions): Promise<ExecutionResult> {
        const uniqueId = getUniqueFileName();
        const tempFileName = uniqueId + ".rb";
        ...
        const dockerArgs = [
            ...
            '--mount', `type=bind,source=${tempFilePath},target=/code.rb,readonly`,  
            RUBY_IMAGE_TAG,
            'ruby', '/code.rb'  // command to execute in the shell after the image is built
        ];
